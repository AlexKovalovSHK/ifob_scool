...
import api, { userApi } from "../../features/auth/api"
import {
  Container,
  Typography,
  Box,
...
export const UserCabinet = () => {
  ...
  const { data: videoList = [], isLoading: videosLoading } = useQuery<
    VideoItem[]
  >({
    queryKey: ["videos"],
    queryFn: async () => {
    ...
    setPurchasedCourses([
      {
        ...
        progress: 65,
        lastLesson: "II закон инструментовки",
        instructor: "Парафейник Максим",
      },
    ])
  }, [])
  ...
  const handleTelegramAuth = (tgUser: TelegramAuthData) => {
    ...
    // Теперь вызываем метод через API напрямую или через новый thunk
    userApi
      .linkTelegram(tgUser)
      .then(updatedUser => {
        alert("Telegram успешно привязан!")
        // Обязательно обновите данные в Redux, чтобы UI увидел изменения
        dispatch(fetchUser(user.id))
      })
      .catch(err => alert("Ошибка привязки: " + err.response?.data?.message))
  }
